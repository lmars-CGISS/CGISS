#ifndef _ALTOVA_INCLUDED_gie_ALTOVA_xs_ALTOVA_CanyType
#define _ALTOVA_INCLUDED_gie_ALTOVA_xs_ALTOVA_CanyType



namespace gie
{

namespace xs
{	

class CanyType : public TypeBase
{
public:
	gie_EXPORT CanyType(xercesc::DOMNode* const& init);
	gie_EXPORT CanyType(CanyType const& init);
	void operator=(CanyType const& other) { m_node = other.m_node; }
	static altova::meta::ComplexType StaticInfo() { return altova::meta::ComplexType(types + _altova_ti_xs_altova_CanyType); }
	gie_EXPORT void operator=(const string_type& value);
	gie_EXPORT operator string_type();
	gie_EXPORT void SetXsiType();
};



} // namespace xs

}	// namespace gie

#endif // _ALTOVA_INCLUDED_gie_ALTOVA_xs_ALTOVA_CanyType
