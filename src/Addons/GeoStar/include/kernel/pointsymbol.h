...
/// \brief 点符号
class GS_API GsPointSymbol:public GsSymbol
{
protected:
	/// \brief 大小
	double m_dblSize;
...
public:
	...
	virtual GsBox Envelope()
	{
		...
		{
			float f[8] = {-m_dblLongAxis,-m_dblShortAxis,
...
public:
	...
	/// \brief 获取节点类型数组
	void CurvePoints(GsVector<GsRawPoint> &vecPoints)
	{
		if(vecPoints.empty())
	...
	/// \brief 获取的节点类型数组
	GsVector<GsRawPoint>  CurvePoints()
	{
		return m_vecPoints;
	}
...
public:
	...
	GsCurvelinePointSymbol(GsVector<GsRawPoint>& vec);
	GsCurvelinePointSymbol(GsVector<GsRawPoint>& vec,const GsColor& c);
...
public:
	...
	/// \brief 从颜色和size构造
	GsPolygonPointSymbol(GsVector<GsRawPoint>& vec,const GsColor& c);
...
public:
	...
	GsCurvegonPointSymbol(GsVector<GsRawPoint>& vec);
	GsCurvegonPointSymbol(GsVector<GsRawPoint>& vec,const GsColor& c);
	~GsCurvegonPointSymbol();
protected:
	/// \brief 当绘制点的时候发生
	/// \details 子类通过覆盖此函数实现点的绘制。基类已经实现了地理坐标到屏幕坐标的转换
...
public:
	...
	/// \brief 从颜色和size构造
	GsPolylinePointSymbol(GsVector<GsRawPoint>& vec,const GsColor& c);
