...
{
public:
	GsBrushStylePixels(GsBrushStyle eStyle, GsColor &c);
...
protected:
	...
	void WriteColor(GsColor &c, unsigned char* pStream);
};
...
protected:
	...
	/// \brief 画笔的模板
	std::vector<float> m_vecDashPattern;
	/// \brief 画笔偏移长度
	float m_fDashOffset;
	/// \brief 线衔接尖头限制
	float m_fMiterLimit;
...
public:
	...
	/// \brief 设置自定义点划线的模板
	virtual void DashPattern(const GsVector<float>& pattern);
	/// \brief 自定义点划线的模板
	virtual GsVector<float> DashPattern() const;
...
public:
    ...
    /// \return 返回绘制是否成功
	virtual bool DrawImage(Utility::GsImage* pImg,const Utility::GsPT& pt = Utility::GsPT());
...
public: 
	...
	/// \returns 返回绘制是否成功
	virtual bool DrawImage(Utility::GsImage* pImg,const Utility::GsRect& source,const Utility::GsRectF& target) = 0;
